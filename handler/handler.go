...
func (h *HttpHandlerImpl) BindUint64Param(w http.ResponseWriter, r *http.Request, key string, data *uint64) bool {
	...
	if param == "" {
		h.Fail(w, r, msg.NewError(reason.MissingRequiredParam).WithReasonTemplateData(reason.ParamTd{Param: key}), nil)
		return true
	}
	id, err := strconv.ParseUint(param, 10, 64)
	if err != nil {
		h.Fail(w, r, msg.NewError(reason.InvalidUintParam).WithReasonTemplateData(reason.ParamTd{Param: key}), nil)
		return true
	}
	data = &id
	return false
