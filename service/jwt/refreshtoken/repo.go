...
func (s *repo) Save(ctx context.Context, tx *gorm.DB, refreshToken *RefreshToken) (dupLogin bool, err error) {
...
func (s *repo) Update(ctx context.Context, tx *gorm.DB, partialUpdatedModel *RefreshToken) (updatedModel *RefreshToken, err error) {
	...
	if result.RowsAffected == 0 {
		return updatedModel, model.ErrModified("RefreshToken")
	}
	err = tx.First(&updatedModel, "id = ?", partialUpdatedModel.Id).Error
	return
}

func (s *repo) Delete(ctx context.Context, tx *gorm.DB, userId uint, loginDeviceType useragent.DeviceType) error {
	return tx.
		Where(&RefreshToken{
			UserId:          &userId,
...
func (s *repo) Get(ctx context.Context, tx *gorm.DB, userId uint, loginDeviceType useragent.DeviceType) (refreshToken string, err error) {
