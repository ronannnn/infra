...
func parseVersion(ver string, versionNo *VersionNo) {
	var err error
	parts := strings.Split(ver, ".")
	if len(parts) > 0 {
		if versionNo.Major, err = strconv.Atoi(parts[0]); err != nil {
			return
		}
	}
	if len(parts) > 1 {
		if versionNo.Minor, err = strconv.Atoi(parts[1]); err != nil {
			return
		}
		if len(parts) > 2 {
			if versionNo.Patch, err = strconv.Atoi(parts[2]); err != nil {
				return
			}
		}
	}
